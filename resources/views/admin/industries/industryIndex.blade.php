...
@section('content')
    <div class="card shadow">
        <div class="card-header">
            <a href="{{route('admin.industries.create')}}" class="btn btn-primary mr-1">{{__('admin.create')}}</a>
        </div>
        <div class="card-body">
            ...
            <table class="table table-striped">
                ...
                @foreach($industries as$industry)
                    <tr>
                        ...
                        <td>
                            <a href="{{route('admin.industries.show',$industry)}}">{{ str_repeat("~",$industry->depth).$industry->name }}</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
        ...
        <div class="card-footer d-flex justify-content-between">
            {{$industries->links()}}
            <span class="align-self-center">views: {{$industries->count()}}, total: {{$industries->total()}}</span>
        </div>
    </div>
@endsection
